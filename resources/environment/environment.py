...
def initialize(self:sim, scene:ui.scene):
    ...
    self.graspableObjects.append(scene.box(0.1, 0.1, 0.1).material('#ffffff', 1))
    self.graspableObjects[-1].move(0.5, 0.0, 0.0)
...
def initializeGripper(self:sim, scene:ui.scene):
    ...
    """
    pass
def update(self:sim, scene:ui.scene, robotServer, robotModel):
    """ 
    ...
    for obj in self.graspableObjects:
        obj.rotate(time.time(), 0, 0)
        obj.move((time.time()-self.environmentData['startTime'])/40)
        obj.scale(1+math.sin(time.time()))
    pass